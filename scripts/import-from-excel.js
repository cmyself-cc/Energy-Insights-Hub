...
const LIMIT = parseInt(process.env.LIMIT) || 200; // 限制处理数量，默认 200
...
async function main() {
  console.log('Starting import from Excel with LLM processing...');
  ...
  console.log(`Processing ${toProcess.length} records (most recent)`);
  ...
  for (let i = 0; i < toProcess.length; i++) {
    ...
    try {
      console.log(`\n[${i+1}/${toProcess.length}] Processing: ${record.title.substring(0, 50)}...`);
      ...
      console.log(`  ✓ Categories: ${JSON.stringify(llmResult.categories)}`);
